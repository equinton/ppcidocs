...
- isdoc="1" : le lien va être générer en utilisant le radical décrit dans *app.docroot* concaténé au contenu de l'attribut *module* (ajout automatique du /), et déclenchera l'affichage dans un nouvel onglet. Ce mécanisme est utilisé pour la documentation externe.
...
- isdoc="1": the link will be generated using the root described in *app.docroot* concatenated with the content of the *module* attribute (automatic addition of /), and will trigger display in a new tab. This mechanism is used for external documentation.
